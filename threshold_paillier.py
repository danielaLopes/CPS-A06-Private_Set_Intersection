...
class ThresholdPaillier(object):
    def __init__(self,size_of_n,n_shares):
        #size_of_n = 1024
...
class ThresholdPaillierPrivateKey(object):
    ...
    def partialDecrypt(self, c):
        return pow(c.c, self.si*2*self.delta, self.nSPlusOne)
...
def combineShares(shrs, w, delta, combineSharesConstant, nSPlusOne, n, ns, serverIds):
        cprime = 1
        for i in range(w):
            ld = delta
            for iprime in range(w):
                if i != iprime:
                    if serverIds[i] != serverIds[iprime]:
                        ld = (ld * - serverIds[iprime]) // (serverIds[i] - serverIds[iprime])

            shr = sympy.mod_inverse(shrs[i], nSPlusOne) if ld < 0 else shrs[i]
